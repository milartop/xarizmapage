(() => {
    ...
    (function(global, factory) {
        ...
        function extend$1(target, src) {
            ...
            Object.keys(src).forEach((key => {
                if ("undefined" === typeof target[key]) target[key] = src[key]; else if (isObject$1(src[key]) && isObject$1(target[key]) && Object.keys(src[key]).length > 0) extend$1(target[key], src[key]);
            }));
        }
        const ssrDocument = {
            body: {},
        ...
        function updateClickedSlide(e) {
            ...
            if (slide && slideFound) {
                ...
                if (swiper.virtual && swiper.params.virtual.enabled) swiper.clickedIndex = parseInt($(slide).attr("data-swiper-slide-index"), 10); else swiper.clickedIndex = slideIndex;
            } else {
        ...
        function onTouchMove(event) {
            ...
            if (data.isTouchEvent && document.activeElement) if (e.target === document.activeElement && $(e.target).is(data.focusableElements)) {
                data.isMoved = true;
        ...
        function Mousewheel(_ref) {
            ...
            function handle(event) {
                ...
                if ("container" !== swiper.params.mousewheel.eventsTarget) target = $(swiper.params.mousewheel.eventsTarget);
    ...
    new Swiper(".holl1__slider", {
        ...
        pagination: {
            ...
            type: "fraction",
            renderFraction: function(currentClass, totaltClass) {
                return '<span class=" ' + currentClass + ' "></span> ' + "из " + '<span class=" ' + totaltClass + ' "></span>';
            }
        },
        navigation: {
    ...
    new Swiper(".holl3__slider", {
        ...
        navigation: {
            prevEl: ".swiper-button-prevs3",
            nextEl: ".swiper-button-nexts3"
        },
        breakpoints: {
            279: {
                slidesPerView: 1
    ...
    new Swiper(".booking__slider", {
        ...
        navigation: {
            prevEl: ".swiper-button-prevs5",
            nextEl: ".swiper-button-nexts5"
        },
        breakpoints: {
            290: {
                slidesPerView: 1,
    ...
    function toggleActiveAccordion(e, header) {
        ...
        if (clickedAccordion.classList.contains("active")) accordionBody.style.maxHeight = accordionBody.scrollHeight + "px"; else accordionBody.style.maxHeight = 0;
    }
    ...
    let arrowOp = document.querySelector(".footer__arrow");
    window.addEventListener("scroll", arrScroll);
    function arrScroll() {
        let windowScroll = document.body.scrollTop || document.documentElement.scrollTop;
        if (windowScroll >= 700) arrowOp.style.display = "block"; else arrowOp.style.display = "none";
    }
    let buttonHeaderHoll = document.querySelector(".button-header__holl");
    buttonHeaderHoll.addEventListener("click", (function() {
        document.documentElement.classList.remove("menu-open");
        document.documentElement.classList.remove("lock");
    }));
    let portfolioLayer = document.querySelectorAll(".parallax-img");
    ...
    const headerDescr = document.querySelector(".top-left__descr");
    const divideText = itm => {
        itm.innerHTML = itm.textContent.replace(/(\S*)/g, (m => `<div class="word">` + m.replace(/(-|#|@)?\S(-|#|@)?/g, "<div class='letter'>$&</div>") + `</div>`));
        return itm;
    };
    const divide = divideText(headerDescr);
    function random(min, max) {
        return Math.random() * (max - min) + min;
    }
    Array.from(divide.querySelectorAll(".letter")).forEach(((el, ind) => {
    ...
    fnTitle(".gs-title-holl");
    fnTitle(".gs-title-benefits");
    fnTitle(".gs-title-booking");
    fnTitle(".gs-title-answer");
    function descr(elem) {
        const gsDescr = document.querySelector(elem);
        const divideTextHall = itm => {
            itm.innerHTML = itm.textContent.replace(/(\S*)/g, (m => `<div class="word">` + m.replace(/(-|#|@)?\S(-|#|@)?/g, "<div class='letter'>$&</div>") + `</div>`));
            return itm;
        };
        const divideHall = divideTextHall(gsDescr);
        Array.from(divideHall.querySelectorAll(".letter")).forEach(((el, ind) => {
            TweenMax.from(el, 2, {
                scrollTrigger: {
                    trigger: elem,
                    start: "top 90%",
    ...
    descr(".gs-descr-hall");
    descr(".gs-descr-benefits");
    descr(".gs-descr-booking");
    descr(".gs-descr-answer");
    let gsAccordion = document.querySelectorAll(".gs-accordion");
    gsAccordion.forEach((function(elem) {
        gsap.to(elem, {
            scrollTrigger: {
                trigger: elem,
                start: "top 75%",
    ...
    let gsBenefits = document.querySelectorAll(".gs-benefits");
    gsBenefits.forEach((function(elem) {
        gsap.to(elem, {
            scrollTrigger: {
                trigger: elem,
                start: "top 85%",
